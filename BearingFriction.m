...
if(type=="deepBall")
    ...
    s1=3.73e-3;
    s2=36.5;
    grr=r1*dm^1.6 *(fr+r2*fa/(sind(24.6*(fa/statLoad)^0.24)))^0.54;
    uehl=0.04;
    gsl=s1*dm^(-0.145)*(fr^5 +(s2*dm^1.5 *fa^4)/(sind(24.6*(fa/statLoad))))^0.33;
...
elseif(type=="angularBall")
    ...
    s2=0.71;
    s3=1.84e-12;
    grr=r1*dm^1.97 *(fr+r3*dm^3.5*RPM^2+r2*fa)^0.54;
    uehl=0.04;
    gsl=s1*dm^0.26 *((fr+s3*dm^3.5*RPM^2)^1.33 +s2*fa^1.33);
    kball=kz*(ID+OD)/(OD-ID) *10^-12;
    fa=0.05*kz*(ID+OD)/(OD-ID);
    Rs=0.36*dm^2*fa;
    mdrag = 0.4*getVm(0,dm)*kball*dm^5 *RPM^2 + 1.093e-7*RPM^2 *dm^3 *(RPM*dm^2 /oilViscosity)^-1.379 *Rs;
...
elseif(type=="taperRoller")
    ...
    s1=0.15;
    s2=2;
    grr=r1*dm^2.38 *(fr+r2*1.6*fa)^0.31;
    uehl=0.002;
    gsl=s1*dm^0.82 *(fr+s2*1.6*fa);
    kroll=kl*kz*(ID+OD)/(OD-ID);
    fa=0.05*kz*(OD+ID)/(OD-ID);
    Rs=0.36*dm^2 *fa;
    id=5*kl*width/dm;
...
elseif(type=="sphericalRoller")
    ...
    s3=2.1e-2;
    s4=41;
    grr=min(r1*dm^1.85 *(fr+r2*fa)^0.54,r3*dm^2.3 *(fr+r4*fa)^0.31);
    uehl=0.04;
    gsl = min(s1*dm^0.25 *(fr^4 +s2*fa^4)^0.33,s3*dm^0.94 *(fr^3 +s4*f(a)^3)^0.33);
    kroll=kl*kz*(ID+OD)/(OD-ID);
    fa=0.05*kz*(OD+ID)/(OD-ID);
    Rs=0.36*fm^2 *fa;
    id=5*kl*width/dm;
